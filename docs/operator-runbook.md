...
SMTP delivery is implemented through an isolated Nodemailer transport. The action stores only the hashed 30-minute token, returns the same generic result for every address, and schedules delivery after the response. Email copy follows the active account's saved English or Brazilian Portuguese locale. A failure writes only `PASSWORD_RESET_EMAIL_DELIVERY_FAILED`; do not change this path to log the caught provider error, recipient, token, mailbox password, database URL, or session secret.

Use `smtp.hostinger.com` with either port 465 and `SMTP_SECURE=true`, or port 587 and `SMTP_SECURE=false` (the app requires STARTTLS). `SMTP_USER` is the full mailbox address. `APP_URL` must be the final HTTPS origin with no path. Keep the `SMTP_PASSWORD` value inside the template's single quotes so Compose treats special password characters literally. The application container receives these values from `/etc/school-records-platform/production.env`; they are not GitHub deployment secrets and must not be committed.

Before launch, verify the domain's SPF, DKIM, and DMARC status in the school-owned email/DNS panels. Then use active test accounts saved once in English and once in Brazilian Portuguese to submit `/forgot-password`. Confirm both requests show the same generic browser result, each inbox receives the correct localized message, each link starts with the exact `APP_URL`, and the link opens the reset form and changes the password. Also submit an unknown address and confirm the browser result is identical; do not expect a message. Record date, operator, sender, recipient provider, locale, and pass/fail without recording the link or token. Local fake-transport tests are not a substitute for this check.

## Launch checklist and recurring maintenance

Before launch, confirm all of the following: school ownership is recorded; public DNS/TLS works; only SSH/80/443 are open; PostgreSQL has no public port; production secrets are protected; GitHub production approval works; a deployment passed; English and Brazilian Portuguese Hostinger password-reset delivery passed with SPF/DKIM/DMARC verified; an external uptime alert works; Hostinger backups are enabled; an off-VPS dump/checksum exists; a real restore rehearsal passed; incident contacts exist; and the normal admin/teacher/reception smoke test passes in both supported locales.
